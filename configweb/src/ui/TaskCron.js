...
function TaskCron() {
    ...
    const handleCronChange = (index, key, value) => {
        const newTaskCronList = [...taskCronList];
        newTaskCronList[index][key] = value;
        setConfig({
            ...config,
    ...
    const handleCommandChange = (taskIndex, commandIndex, key, value) => {
        const newTaskCronList = [...taskCronList];
        newTaskCronList[taskIndex].commands[commandIndex][key] = value.split(',').map(item => item.trim());
        setConfig({
            ...config,
            task_cron_list: newTaskCronList,
        });
                            ...
                            {task.commands.map((command, commandIndex) => (
                                <div key={commandIndex}>
                                    <input
                                        ...
                                        onChange={e => handleCommandChange(taskIndex, commandIndex, 'args', e.target.value)}
                                    />
                                </div>
                            ))}
                        </div>
