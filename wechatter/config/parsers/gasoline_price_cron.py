...
def parse_gasoline_price_cron_rules(
        gasoline_price_cron_rules: List,
) -> List[CronTask]:
    """
    解析汽油价格定时任务规则
    :param gasoline_price_cron_rules: 汽油价格定时任务规则
    :return cron_tasks: 汽油价格定时任务列表
    """
    cron_tasks = []
    for rule in gasoline_price_cron_rules:
        cron = rule["cron"]
        tasks = rule["tasks"]
        trigger = CronTrigger(
            year=cron["year"],
        ...
        def func():
            for task in tasks:
                ...
                send_message = SendMessage(
                    type=SendMessageType.TEXT, content=get_gasoline_price_str(task["city"])
                )
